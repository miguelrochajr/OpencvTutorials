...
void showHistoCallback(int state, void* userData)
{
	...
	vector<Mat> bgr; // This is a vector of Mats

	split(img, bgr); //Split the original img into the channels. Here the vector
									 //will have 3 images (Mat objects)
	...
	Mat b_hist, g_hist, r_hist; // Each of this Images will be

	calcHist(&bgr[0], 1, 0, Mat(), b_hist, 1, &numbins, &histRange); // Blue channel
	calcHist(&bgr[1], 1, 0, Mat(), g_hist, 1, &numbins, &histRange); // Green channel
	calcHist(&bgr[2], 1, 0, Mat(), r_hist, 1, &numbins, &histRange); // Red channel
...
void lomoCallback(int state, void* userData)
{
	...
	vector<Mat> bgr;
	split(img, bgr);
	/* LUT FUNCTIONS PARAMETES */
	// An input image, a matrix of a lookup table, an output image
	LUT(bgr[2], lut, bgr[2]); // LUT in red channels
